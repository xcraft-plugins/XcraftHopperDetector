    ...
    protected boolean execute(final CommandSender sender, final MatchResult[] data) throws UsageException {
        ...
        if (startIndex >= actList.size()) {
            ...
            for (int i = startIndex, e = Math.min(startIndex + pageSize, actList.size()); i < e; ++i) {
                this.displayLine(toSender, actList, i);
            }
        }
        return true;
    }
    
    private void displayLine(final IOutput toSender, final List<Map.Entry<Location, AtomicInteger>> actList, final int i) {
        final Map.Entry<Location, AtomicInteger> entry = actList.get(i);
        final Location l = entry.getKey();
        toSender.output(String.format(ChatColor.YELLOW.toString() + "%d" + ChatColor.WHITE + ". " + ChatColor.GREEN + "(%d, %d, %d) %s " + ChatColor.DARK_GREEN + "%d", i + 1, l.getBlockX(), l.getBlockY(), l.getBlockZ(), l.getWorld().getName(), entry.getValue().get()));
    }
}
